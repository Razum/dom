...
the service worker will be uninstalled. Note that depending on how `/service-ws.worker.js` is served,
...
`service-ws.worker.js` [with HTTP caching disabled](http://stackoverflow.com/questions/38843970/service-worker-javascript-update-frequency-every-24-hours).
If that's not possible—[GitHub Pages](#github-pages), for instance, does not
allow you to change the default 10 minute HTTP cache lifetime—then be aware
that if you visit your production site, and then revisit again before
`service-ws.worker.js` has expired from your HTTP cache, you'll continue to get
...
IMPORTANT: you need to set proper HTTP caching headers for `service-ws.worker.js` file in `firebase.json` file or you will not be able to see changes after first deployment ([issue #2440](https://github.com/facebookincubator/create-react-app/issues/2440)). It should be added inside `"hosting"` key like next:
...
```
{
  "hosting": {
    ...
    "headers": [
      {"source": "/service-ws.worker.js", "headers": [{"key": "Cache-Control", "value": "no-cache"}]}
    ]
    ...
```
